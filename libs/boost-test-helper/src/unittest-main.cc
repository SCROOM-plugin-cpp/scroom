...
int main(int argc, char* argv[])
{
  ...
  if (argc > 1)
    std::cerr << "You have requested XML output. Your command-line arguments will be ignored"
              << std::endl;
  ...
  const char* alternative[] = {
      "--log_format=XML",    "--log_level=all",   outputArgumentString.c_str(),
      "--output_format=XML", "--report_level=no",
  };
  int count = sizeof(alternative) / sizeof(alternative[0]);

  return ::boost::unit_test::unit_test_main(&init_unit_test, count,
                                            const_cast<char**>(alternative));
#else
