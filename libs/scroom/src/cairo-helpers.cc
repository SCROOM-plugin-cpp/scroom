...
void drawOutOfBoundsWithBackground(cairo_t* cr, Rectangle<double> const& requestedPresentationArea,
...
void drawOutOfBoundsWithBackgroundColor(cairo_t* cr, const Color& background,
                                        ...
                                        double pixelSize)
{
  drawOutOfBoundsWithoutBackground(cr, requestedPresentationArea, actualPresentationArea,
                                   pixelSize);
                                      ...
                                      double pixelSize)
{
  std::list<Rectangle<double> > border =
      boost::assign::list_of(requestedPresentationArea.leftOf(actualPresentationArea.getLeft()))(
  ...
  for (const Rectangle<double>& r : border)
  {
    if (!r.isEmpty())
    {
      drawRectangle(cr, Colors::OUT_OF_BOUNDS,
                    pixelSize * (r - requestedPresentationArea.getTopLeft()));
    }
  }
}
