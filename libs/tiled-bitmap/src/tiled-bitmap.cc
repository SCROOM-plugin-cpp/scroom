...
static Scroom::MemoryBlobs::PageProvider::Ptr createProvider(double width, double height, int bpp)
{
  ...
  size_t pagesPerBlock =
      static_cast<size_t>(std::max(ceil(guessedTileSizeAfterCompression / 10.0 / pagesize_d), 1.0));
...
class LoadOperation : public FileOperation
{
  ...
  public:
  static Ptr create(ThreadPool::WeakQueue::Ptr queue, Layer::Ptr const& l,
                    SourcePresentation::Ptr sp, ProgressInterface::Ptr progress);
...
FileOperation::Ptr LoadOperation::create(ThreadPool::WeakQueue::Ptr queue, Layer::Ptr const& l,
...
LoadOperation::LoadOperation(ThreadPool::WeakQueue::Ptr queue_, Layer::Ptr const& l,
                             SourcePresentation::Ptr sp, ProgressInterface::Ptr progress_)
...
void TiledBitmap::initialize()
{
  ...
  do
  {
    ...
    Layer::Ptr layer = Layer::create(shared_from_this<TiledBitmap>(), static_cast<int>(i), width,
                                     height, bpp, provider);
...
void TiledBitmap::drawTile(cairo_t* cr, const CompressedTile::Ptr tile,
                           const Scroom::Utils::Rectangle<double> viewArea)
{
  ...
  {
    Scroom::Utils::Rectangle<double> rect(viewArea.x() + margin, viewArea.y() + margin,
                                          viewArea.width() - 2 * margin,
                                          viewArea.height() - 2 * margin);

    cairo_set_source_rgb(cr, 0, 0, 0);  // Black
    drawRectangleContour(cr, rect);
    char buffer[256];
    snprintf(buffer, 256, "Layer %d, Tile (%d, %d), %d bpp", tile->depth, tile->x, tile->y,
             tile->bpp);
...
void TiledBitmap::redraw(ViewInterface::Ptr const& vi, cairo_t* cr,
                         Scroom::Utils::Rectangle<double> const& presentationArea, int zoom)
{
  ...
  const auto validPresentationArea =
      scaledRequestedPresentationArea.intersection(actualPresentationArea);
  ...
  const auto clippedRequestedPresentationArea =
      scaledRequestedPresentationArea.above(validPresentationArea.getBottom())
  ...
  for (int i = imin; i < imax; i++)
  {
    for (int j = jmin; j < jmax; j++)
    {
      ...
      const auto viewAreaRect =
          (visibleTileArea - clippedRequestedPresentationArea.getTopLeft()) * pixelSize;
