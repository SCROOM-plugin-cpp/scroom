...
void TileViewState::setViewData(TiledBitmapViewData::Ptr tbvd_)
{
  ...
  if (!lifeTimeManager_)
  {
    lifeTimeManager_ = boost::shared_ptr<void>(
...
void TileViewState::kick()
{
  ...
  if (state >= LOADED && desiredState >= state && !queue && tbvd_)
  {
    ...
    cpuBound->schedule(
        boost::bind(&TileViewState::process, shared_from_this<TileViewState>(), weakQueue),
        LOAD_PRIO, queue);
  }
}
...
void TileViewState::process(ThreadPool::WeakQueue::Ptr wq)
{
  for (;;)
  {
    ...
    if (wq == weakQueue && state < desiredState)
    {
      ...
      {
      case LOADED:
        fn = boost::bind(&TileViewState::computeBase, shared_from_this<TileViewState>(), wq, tile,
                         lo);
        state = COMPUTING_BASE;
        break;

      case BASE_COMPUTED:
        fn = boost::bind(&TileViewState::computeZoom, shared_from_this<TileViewState>(), wq, tile,
                         lo, baseCache, zoom);
...
void TileViewState::computeBase(ThreadPool::WeakQueue::Ptr wq, ConstTile::Ptr tile_,
                                LayerOperations::Ptr lo_)
{
  ...
  //        parent->depth, parent->x, parent->y, state, desiredState, (bool)tbvd,
  //        (bool)(wq==weakQueue));
...
void TileViewState::computeZoom(ThreadPool::WeakQueue::Ptr wq, ConstTile::Ptr tile_,
                                LayerOperations::Ptr lo_, Scroom::Utils::Stuff baseCache_,
                                int zoom_)
{
  ...
  //        parent->depth, parent->x, parent->y, state, desiredState, zoom, this->zoom, (bool)tbvd,
  //        (bool)(wq==weakQueue));
...
void TileViewState::reportDone(ThreadPool::WeakQueue::Ptr wq, ConstTile::Ptr tile_)
{
  ...
  for (TileLoadingObserver::Ptr observer :
       Scroom::Utils::Observable<TileLoadingObserver>::getObservers())
  {
