...
PluginManager::PluginManager() : devMode(false), state(FINDING_DIRECTORIES) {}
...
#ifdef _WIN32
    // We want to keep everything portable on windows so we look for the plugin folder in the same
    // directory as the .exe
    std::string plugin_path =
        (boost::dll::program_location().parent_path() / "plugins").generic_string();
...
void PluginManager::registerNewPresentationInterface(
    const std::string& identifier, NewPresentationInterface::Ptr newPresentationInterface)
{
...
void PluginManager::registerOpenPresentationInterface(
    const std::string& extension, OpenPresentationInterface::Ptr openPresentationInterface)
{
...
const std::map<NewPresentationInterface::Ptr, std::string>&
...
const std::map<OpenPresentationInterface::Ptr, std::string>&
