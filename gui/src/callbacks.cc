...
{
  NewPresentationInterface* newPresentationInterface =
      static_cast<NewPresentationInterface*>(user_data);
...
void on_open_activate(GtkMenuItem*, gpointer user_data)
{
  ...
  dialog = gtk_file_chooser_dialog_new(
      "Open File", GTK_WINDOW(scroom), GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL,
  ...
  const std::map<OpenInterface::Ptr, std::string>& openInterfaces =
      PluginManager::getInstance()->getOpenInterfaces();
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT)
  {
    ...
    GFileInfo* fileInfo =
        g_file_query_info(file, "standard::*", G_FILE_QUERY_INFO_NONE, NULL, NULL);
    ...
    filterInfo.contains = static_cast<GtkFileFilterFlags>(
...
#ifdef _WIN32
  // We want to keep everything portable on windows so we look for the .glade file in the same
...
void create_scroom(PresentationInterface::Ptr presentation)
{
  ...
  g_signal_connect(static_cast<gpointer>(closeMenuItem), "activate", G_CALLBACK(on_close_activate),
                   view.get());
  g_signal_connect(static_cast<gpointer>(quitMenuItem), "activate", G_CALLBACK(on_quit_activate),
                   view.get());
  g_signal_connect(static_cast<gpointer>(openMenuItem), "activate", G_CALLBACK(on_open_activate),
                   scroom);
  g_signal_connect(static_cast<gpointer>(fullScreenMenuItem), "activate",
                   G_CALLBACK(on_fullscreen_activate), view.get());
  g_signal_connect(static_cast<gpointer>(zoomBox), "changed", G_CALLBACK(on_zoombox_changed),
                   view.get());
  g_signal_connect(static_cast<gpointer>(vscrollbaradjustment), "value-changed",
                   G_CALLBACK(on_scrollbar_value_changed), view.get());
  g_signal_connect(static_cast<gpointer>(hscrollbaradjustment), "value-changed",
                   G_CALLBACK(on_scrollbar_value_changed), view.get());
  g_signal_connect(static_cast<gpointer>(xTextBox), "changed", G_CALLBACK(on_textbox_value_changed),
                   view.get());
  g_signal_connect(static_cast<gpointer>(yTextBox), "changed", G_CALLBACK(on_textbox_value_changed),
                   view.get());
  ...
  //                   view.get());
  g_signal_connect(static_cast<gpointer>(aboutMenuItem), "activate", G_CALLBACK(on_about_activate),
                   view.get());
  g_signal_connect(static_cast<gpointer>(drawingArea), "expose_event",
                   G_CALLBACK(on_drawingarea_expose_event), view.get());
  g_signal_connect(static_cast<gpointer>(drawingArea), "configure_event",
                   G_CALLBACK(on_drawingarea_configure_event), view.get());
  g_signal_connect(static_cast<gpointer>(drawingArea), "button-press-event",
                   G_CALLBACK(on_button_press_event), view.get());
  g_signal_connect(static_cast<gpointer>(drawingArea), "button-release-event",
                   G_CALLBACK(on_button_release_event), view.get());
  g_signal_connect(static_cast<gpointer>(drawingArea), "scroll-event", G_CALLBACK(on_scroll_event),
                   view.get());
  g_signal_connect(static_cast<gpointer>(drawingArea), "motion-notify-event",
                   G_CALLBACK(on_motion_notify_event), view.get());
}
...
void on_presentation_created(PresentationInterface::Ptr presentation)
{
  ...
  std::map<PresentationObserver::Ptr, std::string>::const_iterator cur =
      presentationObservers.begin();
  std::map<PresentationObserver::Ptr, std::string>::const_iterator end =
      presentationObservers.end();
  for (; cur != end; cur++) cur->first->presentationAdded(presentation);
}
...
void on_view_created(View::Ptr v)
{
  ...
  for (; cur != end; cur++) t.add(cur->first->viewAdded(v));
}
...
void on_presentation_possibly_destroyed()
{
  ...
  {
    for (auto const& p : views) p.first->on_presentation_destroyed();

    const std::map<PresentationObserver::Ptr, std::string>& presentationObservers =
        PluginManager::getInstance()->getPresentationObservers();

    for (auto const& p : presentationObservers) p.first->presentationDeleted();
  }
}
