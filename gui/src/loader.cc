...
class GtkFileFilterListDestroyer
{
  ...
  public:
  GtkFileFilterListDestroyer(std::list<GtkFileFilter*>& f) : filters(f)
  {
    for (auto const filter : filters) g_object_ref_sink(filter);
  }
...
GtkFileFilterInfoPtr filterInfoFromPath(const std::string& filename)
{
  ...
  if (fileInfo)
  {
    ...
    std::unique_ptr<gchar> mime_type(charpFromString(
        g_content_type_get_mime_type(g_file_info_get_content_type(fileInfo.get()))));
    std::unique_ptr<gchar> display_name(
        charpFromString(g_file_info_get_display_name(fileInfo.get())));

    filterInfo.reset(
        new GtkFileFilterInfo());  // filterInfo->filename uninitialized, so a delete is dangerous
...
class ScroomInterfaceImpl : public ScroomInterface
{
  ...
  virtual PresentationInterface::Ptr loadPresentation(
      std::string const& name, std::string const& relativeTo = std::string());
...
PresentationInterface::Ptr loadPresentation(GtkFileFilterInfo const& info)
{
  ...
  if (presentation)
  {
    ...
    throw std::invalid_argument("Don't know how to load presentation " +
...
Aggregate::Ptr ScroomInterfaceImpl::newAggregate(std::string const& name)
{
  ...
  std::map<std::string, NewAggregateInterface::Ptr>::const_iterator i =
      newAggregateInterfaces.find(name);
...
PresentationInterface::Ptr ScroomInterfaceImpl::loadPresentation(std::string const& n,
                                                                 std::string const& rt)
{
